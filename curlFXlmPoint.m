function [rv, thetav, phiv] = curlFXlmPoint(r, th, phi, epiNL, sphr)
...
for l = 0:(size(epiNL,1)-1) 
    ...
    coeffang = alphar(l+1,:) .* ( besselj(l-1,alphar(l+1,:)) - besselj(l+1,alphar(l+1,:))) ...
                    + 2 .* besselj(l,alphar(l+1,:));
    max(coeffang)
    %% calculation
    for mm = 1:(2*l+1)
        ...
        rv(l+1,:,mm) = dcoeff .* besselj(m(mm),alphar(l+1,:)) .* ...
            ( coeffr1 .* ylm(1,1,mm) - ...
